...
--LPAD :왼쪽공백을 특정 값으로 채운다 RPAD: 오른쪽부터 채운다.
SELECT LPAD(FIRST_NAME, 8, '*') FROM EMPLOYEES;

--TRIM : 문자열에서 공백 혹은 두번째 매개변수 제거,  RTRIM, LTRIM 있음
SELECT TRIM(' HELLO WORLD ') FROM DUAL;
SELECT LTRIM( 'HELLO WORLD', 'HELLO') FROM DUAL;

--REPLACE : 타겟, 찾을문자, 변경문자
SELECT REPLACE('A,B,C,D,E', 'C', 'c') FROM DUAL;
...
--SELECT SUBSTR(PHONE_NUMBER, 4,3,)
SELECT CONCAT('02.', SUBSTR(PHONE_NUMBER, 5, LENGTH(PHONE_NUMBER)-3)) FROM EMPLOYEES;
...
SELECT RPAD(SUBSTR(FIRST_NAME, 1, 3),LENGTH(FIRST_NAME),'*') AS NAME, RPAD(SALARY, 10, '*') AS SALARY FROM EMPLOYEES WHERE LOWER(JOB_ID) = 'it_prog';

--틀린점
--LOWER, UPPER, INITCAP의 괄호 안에는 작은따옴표 안쓴다.
